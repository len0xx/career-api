...
Метод для получения списка регионов, существующих на «Время Карьеры»

При успешном выполнении регионы возвращаются в виде массива терминов, упорядоченных в алфавитном порядке. Описание объекта "термин" можно посмотреть по ссылке — [terms](https://github.com/len0xx/career-api/blob/main/docs/terms.md)

### Для получения списка регионов необходимо выполнить запрос:
```
POST /api/get-regions/
```
...
* `could not collect the regions` — во время получения регионов на сервере произошла непредвиденная ошибка, пожалуйста, попробуйте повторить запрос позднее
